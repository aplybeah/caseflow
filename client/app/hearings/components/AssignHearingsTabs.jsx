...
const AvailableVeteransTable = ({
  ...
  } else if (_.isEmpty(rows) || _.isEmpty(columns)) {
    return (
      <div>
        <StatusMessage
  ...
  return (
    <div>
      <Link to={`/schedule/docket/${selectedHearingDay.id}`}>
        {`View the Daily Docket for ${moment(
          selectedHearingDay.scheduledFor
        ).format("M/DD/YYYY")}`}
      </Link>
      <QueueTable
        columns={columns}
...
export default class AssignHearingsTabs extends React.Component {
  ...
  getHearingTime = (date, regionalOfficeTimezone) => {
    ...
    return (
      <div>
        {getTime(date)} /<br />
        {getTimeInDifferentTimeZone(date, regionalOfficeTimezone)}
      </div>
    );
  };
  ...
  getCaseDetailsInformation = appeal => {
    if (appeal.attributes.appellantFullName) {
      return `${appeal.attributes.appellantFullName} | ${
        appeal.attributes.veteranFileNumber
      }`;
    }

    return `${appeal.attributes.veteranFullName} | ${
      appeal.attributes.veteranFileNumber
    }`;
  };
  ...
  getAppealDocketTag = appeal => {
    if (appeal.attributes.docketNumber) {
      return (
        <div>
          <DocketTypeBadge
            name={appeal.attributes.docketName}
            number={appeal.attributes.docketNumber}
          />
  ...
  availableVeteransRows = appeals => {
    ...
    return _.map(sortedByAodCavc, (appeal, index) => ({
      ...
      suggestedLocation: this.getSuggestedHearingLocation(
        appeal.attributes.availableHearingLocations
      ),
      time: null,
  ...
  upcomingHearingsRows = hearings => {
    return _.map(hearings, (hearing, index) => ({
      ...
      time: this.getHearingTime(
        hearing.scheduledFor,
        hearing.regionalOfficeTimezone
      )
    }));
  };
      ...
      },
      {
        ...
        valueFunction: appeal => (
          <Link
            ...
            href={(() => {
              const date = moment(selectedHearingDay.scheduledFor).format(
                "YYYY-MM-DD"
              );
  ...
  amaDocketCutoffLineStyle = appeals => {
    ...
    return docketCutoffLineStyle(
      indexOfLine,
      endOfNextMonth.format("MMMM YYYY")
    );
  };
  ...
  render() {
    ...
    const availableSlots =
      _.get(selectedHearingDay, "totalSlots", 0) -
      Object.keys(hearingsForSelected).length;

    const upcomingRows = this.upcomingHearingsRows(
      sortHearings(hearingsForSelected)
    );
    const amaAppeals = _.filter(appealsReadyForHearing, appeal =>
      this.isAmaAppeal(appeal)
    );
